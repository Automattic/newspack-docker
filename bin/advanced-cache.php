...
if (is_readable(dirname(__FILE__) . '/batcache-stats.php') ) {
    include_once dirname(__FILE__) . '/batcache-stats.php';
}
...
class batcache
{
    ...
    var $flush_keys = array( 'host' ); // Keys from $keys used to bust the cache.
    ...
    function configure_groups()
    {
        ...
        if (function_exists('wp_cache_add_global_groups') ) {
            wp_cache_add_global_groups(array($this->group, $this->flush_group));
        }
    }
...
if (file_exists(__DIR__ . '/../../scripts/is-mobile.php') ) {
    include __DIR__ . '/../../scripts/is-mobile.php';
    // Mobile batcache
    if(Batcache_Jetpack_Mobile\jetpack_is_mobile() && isset($_COOKIE['akm_mobile']) ) {
    ...
    if (Batcache_Jetpack_Mobile\User_Agent_Info::is_blackbeberry() ) {
        $batcache->unique['mobile'] = 'blackberry';
    } elseif (Batcache_Jetpack_Mobile\User_Agent_Info::is_WindowsPhone7() ) {
        $batcache->unique['mobile'] = 'windows-phone7';
    } elseif (Batcache_Jetpack_Mobile\User_Agent_Info::is_S60_OSSBrowser() ) { // https://wpcom.trac.automattic.com/browser/trunk/wp-content/mu-plugins/wp-mobile.php#L103
        $batcache->unique['mobile'] = 'dumb';
    } elseif (in_array(Batcache_Jetpack_Mobile\jetpack_is_mobile('smart', true), array( 'iphone', 'ipod' )) ) {
        $batcache->unique['mobile'] = 'iphone';
    } elseif (Batcache_Jetpack_Mobile\jetpack_is_mobile('smart') ) {
        $batcache->unique['mobile'] = 'smart';
    } elseif (Batcache_Jetpack_Mobile\jetpack_is_mobile('dumb') ) {
        $batcache->unique['mobile'] = 'dumb';
    }

    // iPad
    if (Batcache_Jetpack_Mobile\User_Agent_Info::is_ipad() ) {
        if (false !== strpos(strtolower($_SERVER['HTTP_USER_AGENT']), 'safari') ) {
            $batcache->unique['ipad'] = 'ipad-safari';
        } else {
            $batcache->unique['ipad'] = 'ipad';
        }
    } elseif (( new Batcache_Jetpack_Mobile\User_Agent_Info() )->is_tablet() ) {
        // Tablets
...
if (isset($batcache->cache['version']) && $batcache->cache['version'] != $batcache->url_version ) {
    ...
    if (! $is_cached || time() >= $batcache->cache['time'] + $batcache->max_age - $batcache->seconds ) {
        ...
        if ($batcache->requests >= $batcache->times  // visited enough times
            && (       ! $is_cached  // no cache
